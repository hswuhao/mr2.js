...
a.version('20190616')
  .option(
    '-s, --server <server>',
    'Like: 1.2.3.4:5, default choose one from built-in server list',
  )
  .option(
    '-p, --password <password>',
    'Like: xxx, default choose one from built-in server list',
  )
  .option(
    '-D, --serverDomain <serverDomain>',
    'Subdomain, like: xxx, default use your machineid, only works with built-in server',
  )
  .option(
    '-P, --serverPort <serverPort>',
    'Like: 10000, only works with customize server',
  )
  .option('-c, --clientServer <clientServer>', 'Like: 127.0.0.1:3000')
  .option(
    '-d, --clientDirectory <clientDirectory>',
    'Like: /path/to/www. Ignore clientServer',
  )
...
(async () => {
  if (a.list) {
    try {
      var r = await require('superagent').get(
        'https://raw.githubusercontent.com/txthinking/mr2.js/master/server.json',
      );
  ...
  try {
    if (a.server.indexOf('mr2.dev') === -1) {
      ...
      if (a.clientDirectory) {
        var s = execSync(
          `${bin} client -s ${a.server} -p ${a.password} -P ${a.serverPort} --clientDirectory ${a.clientDirectory}`,
          {
            encoding: 'utf8',
          },
        );
      ...
      if (a.clientServer) {
        var s = execSync(
          `${bin} client -s ${a.server} -p ${a.password} -P ${a.serverPort} -c ${a.clientServer}`,
          {
            encoding: 'utf8',
          },
        );
    ...
    if (a.server.indexOf('mr2.dev') !== -1) {
      console.log(
        `https://${a.serverDomain}.${addrToIPPort(a.server)[0]}:${a.port}`,
      );
      if (a.clientDirectory) {
        var s = execSync(
          `${bin} client -s ${a.server} -p ${a.password} -D ${a.serverDomain} --clientDirectory ${a.clientDirectory}`,
          {
            encoding: 'utf8',
          },
        );
      ...
      if (a.clientServer) {
        var s = execSync(
          `${bin} client -s ${a.server} -p ${a.password} -D ${a.serverDomain} -c ${a.clientServer}`,
          {
            encoding: 'utf8',
          },
        );
